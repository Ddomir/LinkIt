  ...
  useEffect(() => {
    const resolve = async () => {
      try {
        ...
        if (invErr || !invite) { setStatus("notfound"); return; }
  ...
  if (status === "notfound") {
    return (
      <div className="w-screen h-screen flex flex-col items-center justify-center gap-4 app-bg room-panel">
        ...
        <button
          onClick={onLogin}
          className="px-6 py-2 rounded-full bg-[#77f298] text-black font-semibold hover:bg-[#5ee07e] transition-colors cursor-pointer"
        >
          Log in
        </button>
      </div>
    ...
    <div className="w-screen h-screen flex" style={{ background: 'var(--surface)' }}>
      ...
      <div className="hidden lg:flex flex-col surface h-full w-72 shrink-0">
        ...
        <div className="flex-1 overflow-auto px-3 py-1">
          <div className="flex flex-col gap-1 text-sm text-white/40 px-2 py-2">
            Viewing as guest
          </div>
        </div>
        ...
        <div className="flex items-center justify-end px-5 py-4">
          <button
            ...
            aria-label="Log in"
            title="Log in to join this room"
          >
      ...
      {/* Room content */}
      <div className="flex-1 min-w-0 h-full overflow-hidden lg:rounded-l-2xl lg:mt-0 mt-[52px]">
